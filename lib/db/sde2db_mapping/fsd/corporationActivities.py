...
class corporationActivities(mapper):
    
    def __init__(self, db: db_utils, log:logger):
        self.db = db
        self.log = log
        self.activities = table_activities()
    ...
    def check(self):
        try:
            ...
            self.check_table(self.activities)
    ...
    def start(self):
        try:
            self.db.table_start_sync(self.activities.table_name)
        except Exception as e:
            method_name = traceback.extract_stack(None, 2)[0][2]
            self.log.critical(f'ERROR in {method_name}: {e}')
    ...
    def run(self, id, row):
        try:
            self.add_activity(self.activities.table_name, self.activities.table_pk, id, row)
        except Exception as e:
            method_name = traceback.extract_stack(None, 2)[0][2]
            self.log.critical(f'ERROR in {method_name}: {e}')
    ...
    def finish(self):
        try:
            self.db.table_finish_sync(self.activities.table_name)
        except Exception as e:
            method_name = traceback.extract_stack(None, 2)[0][2]
            self.log.critical(f'ERROR in {method_name}: {e}')           
    ...
    def add_activity(self, table, pk, id, row):
        try:
            ...
            for column in self.activities.columns:
...
class table_activities:
    def __init__(self):
        self.table_name = 'corporation_activities'
        self.table_pk = 'activity_id'
        self.columns = []
        
        self.columns.append(col('activity_id', 'NUMBER', '#root'))
